...
Installed builds check GitHub Releases on launch. When a newer version exists
RamBo says so, shows what changed, and asks whether to install it.

Answer Yes and the rest happens on its own: it downloads `RamBo-Setup.exe`,
runs it silently and exits, and the installer replaces the files and reopens
RamBo. Progress is reported in the status bar, and the install waits for a
running scan, trim or kill animation to finish so the app never disappears
mid-action. The new build confirms itself on first launch with *Updated to
vX.Y.Z* in the status bar.

**No** skips that version permanently; **Cancel** asks again next launch. An
**UPDATE** button in the header offers it again either way, and also appears
if a download or install fails.
